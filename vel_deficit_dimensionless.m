function [vel_def,wake_half,vel_def_max ] = vel_deficit_dimensionless(eld_x, eld_y,v_inf,v,varargin)
...
% Make dimensionless
s = s ./ vel_def_max;
eld_spline = eld_spline ./ wake_half;
y1 = y1 ./ wake_half;
y2 = y2 ./ wake_half;
...
if plotsOn || savePlots
	...
	scatter(vel_def./vel_def_max, eld_y./wake_half)%plot vel deficit
	hold on;
	...
	title([ 'Dimensionless Velocity Deficit For ' objectType ' at ' velocity ' m/s, ' ...
			xPosition ' mm behind TE']);
	ylabel('$\frac{y}{\delta_{1/2}(x)}$ ');
	xlabel('$ \frac{\Delta u(x,y)}{\Delta u_{max}(x)} $');
	plot(s,eld_spline);
	%plot(s(ind1),y1,'*');
	%plot(s(ind2),y2,'*');
	...
	if savePlots
		print( [ 'VelDeficitDimensionless_Exp' num2str(expNum) ], '-djpeg') % save figure as jpeg TODO: save to specific folder
